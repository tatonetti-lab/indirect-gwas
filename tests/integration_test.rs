...
fn read_projection_matrix() {
    ...
    let path = dir.path();
    let args = utils::setup_test(path, 101, 1000, 10, 100, 10, false);
...
#[test]
fn read_gwas_results_compressed() {
    let dir = tempdir().unwrap();
    let path = dir.path();
